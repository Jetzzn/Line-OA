...
const LINE_CHANNEL_ID = "1657498701";
const LINE_CHANNEL_SECRET = "fb58b0cb83030d320d695dfe49dfc0dc";
const LINE_CHANNEL_ACCESS_TOKEN =
  "VKD1miE1bUmOa7rZSwXx5yhfQoiF7aOzPTdiUr5OKyUshfKZSwlD7PLv1s4xFN447kgnFy5ioNWOqI+WsClClc4kWQZOodJfN4LNz1SeZ9yYYcn1BRAmEBeyofE0BztZUheNFzEoAcCv8VO90rCqjAdB04t89/1O/w1cDnyilFU=";
const LINE_LOGIN_CALLBACK_URL =
  "https://8f70-58-10-56-250.ngrok-free.app/line-login-callback";
const JOTFORM_URL = "https://form.jotform.com/240728660362457";
const LIFF_ID = "1657498709-p6ry1vP1";
const SERVER_URL =
  process.env.SERVER_URL || "https://8f70-58-10-56-250.ngrok-free.app";
...
async function handlePostback(event) {
  ...
  if (postback.data === "action=resend_qr") {
    ...
    console.log(`Unknown postback action: ${postback.data}`);
  }
}
...
async function checkRichMenu(userId) {
  try {
    const response = await axios.get(`https://api.line.me/v2/bot/user/${userId}/richmenu`, {
      headers: { Authorization: `Bearer ${LINE_CHANNEL_ACCESS_TOKEN}` }
    });
    console.log(`Current Rich Menu for user ${userId}:`, response.data);
    return response.data.richMenuId;
  } catch (error) {
    console.error(`Error checking Rich Menu for user ${userId}:`, error.response ? error.response.data : error.message);
    return null;
  }
}app.get("/test-resend-qr/:userId", async (req, res) => {
